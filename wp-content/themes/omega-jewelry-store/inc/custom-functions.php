...
if ( ! function_exists( 'omega_jewelry_store_sub_menu_toggle_button' ) ) :
    ...
    function omega_jewelry_store_sub_menu_toggle_button( $args, $item, $depth ) {

        // Add sub menu toggles to the main menu with toggles
        if ( $args->theme_location == 'omega-jewelry-store-primary-menu' && isset( $args->show_toggles ) ) {
        ...
        }elseif( $args->theme_location == 'omega-jewelry-store-primary-menu' ) {

            if ( in_array( 'menu-item-has-children', $item->classes ) ) {

                $args->before = '<div class="link-icon-wrapper">';
                $args->after  = omega_jewelry_store_get_theme_svg( 'chevron-down' ) . '</div>';
...
if ( ! function_exists( 'omega_jewelry_store_the_theme_svg' ) ):
    ...
    function omega_jewelry_store_the_theme_svg( $svg_name, $return = false ) {

        if( $return ){

            return omega_jewelry_store_get_theme_svg( $svg_name ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in omega_jewelry_store_get_theme_svg();.

        }else{

            echo omega_jewelry_store_get_theme_svg( $svg_name ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in omega_jewelry_store_get_theme_svg();.
...
if ( ! function_exists( 'omega_jewelry_store_get_theme_svg' ) ):
    ...
    function omega_jewelry_store_get_theme_svg( $svg_name ) {

        // Make sure that only our allowed tags and attributes are included.
        $svg = wp_kses(
            Omega_Jewelry_Store_SVG_Icons::get_svg( $svg_name ),
            array(
                'svg'     => array(
                    'class'       => true,
                    'xmlns'       => true,
...
if( !function_exists( 'omega_jewelry_store_post_category_list' ) ) :
    ...
    // Post Category List.
    function omega_jewelry_store_post_category_list( $select_cat = true ){
        ...
        if( $select_cat ){
            ...
            $post_cat_cat_array[''] = esc_html__( '-- Select Category --','omega-jewelry-store' );
...
if( !function_exists('omega_jewelry_store_single_post_navigation') ):
    ...
    function omega_jewelry_store_single_post_navigation(){
        ...
        if( $twp_navigation_type == '' || $twp_navigation_type == 'global-layout' ){
            $twp_navigation_type = get_theme_mod('twp_navigation_type', $omega_jewelry_store_default['twp_navigation_type']);
        }
            ...
            if( $twp_navigation_type == 'theme-normal-navigation' ){ ?>
                ...
                <div class="navigation-wrapper">
                    ...
                    the_post_navigation(array(
                        'prev_text' => '<span class="arrow" aria-hidden="true">' . omega_jewelry_store_the_theme_svg('arrow-left',$return = true ) . '</span><span class="screen-reader-text">' . esc_html__('Previous post:', 'omega-jewelry-store') . '</span><span class="post-title">%title</span>',
                        'next_text' => '<span class="arrow" aria-hidden="true">' . omega_jewelry_store_the_theme_svg('arrow-right',$return = true ) . '</span><span class="screen-reader-text">' . esc_html__('Next post:', 'omega-jewelry-store') . '</span><span class="post-title">%title</span>',
                    )); ?>
                </div>
                <?php
...
if( !function_exists('omega_jewelry_store_footer_content_widget') ):
    ...
    function omega_jewelry_store_footer_content_widget(){
        ...
        $omega_jewelry_store_default = omega_jewelry_store_get_default_theme_options();
        if (is_active_sidebar('omega-jewelry-store-footer-widget-0') ||
            is_active_sidebar('omega-jewelry-store-footer-widget-1') ||
            is_active_sidebar('omega-jewelry-store-footer-widget-2')):
            $x = 1;
            $footer_sidebar = 0;
            do {
                if ($x == 3 && is_active_sidebar('omega-jewelry-store-footer-widget-2')) {
                    $footer_sidebar++;
                }
                if ($x == 2 && is_active_sidebar('omega-jewelry-store-footer-widget-1')) {
                    $footer_sidebar++;
                }
                if ($x == 1 && is_active_sidebar('omega-jewelry-store-footer-widget-0')) {
                    $footer_sidebar++;
                }
                $x++;
            } while ($x <= 3);
            ...
            <div class="footer-widgetarea">
                <div class="wrapper">
                    <div class="column-row">
                        ...
                        <?php if (is_active_sidebar('omega-jewelry-store-footer-widget-0')): ?>
                            <div class="column <?php echo 'column-' . absint($footer_sidebar_class); ?> column-sm-12">
                                <?php dynamic_sidebar('omega-jewelry-store-footer-widget-0'); ?>
                            </div>
                        <?php endif; ?>

                        <?php if (is_active_sidebar('omega-jewelry-store-footer-widget-1')): ?>
                            <div class="column <?php echo 'column-' . absint($footer_sidebar_class); ?> column-sm-12">
                                <?php dynamic_sidebar('omega-jewelry-store-footer-widget-1'); ?>
                            </div>
                        <?php endif; ?>

                        <?php if (is_active_sidebar('omega-jewelry-store-footer-widget-2')): ?>
                            <div class="column <?php echo 'column-' . absint($footer_sidebar_class); ?> column-sm-12">
                                <?php dynamic_sidebar('omega-jewelry-store-footer-widget-2'); ?>
                            </div>
                        <?php endif; ?>
                    ...
                    <div class="column column-9">
                        <div class="footer-credits">
                            ...
                            <div class="footer-copyright">
                                ...
                                <?php
                                $footer_copyright_text = wp_kses_post( get_theme_mod( 'footer_copyright_text', $omega_jewelry_store_default['footer_copyright_text'] ) );
                                    echo esc_html( $footer_copyright_text );
                                    echo '<br>';
                                    echo esc_html__('Theme: ', 'omega-jewelry-store') . 'Omega Jewelry Store ' . esc_html__('By ', 'omega-jewelry-store') . '  <span>' . esc_html__('OMEGA ', 'omega-jewelry-store') . '</span>';
                    ...
                    <div class="column column-3 align-text-right">
                        <a class="to-the-top" href="#site-header">
                            <span class="to-the-top-long">
                                <?php
                                printf( esc_html__( 'To the Top %s', 'omega-jewelry-store' ), '<span class="arrow" aria-hidden="true">&uarr;</span>' );
                                ?>
                            </span>
                            <span class="to-the-top-short">
                                <?php
                                printf( esc_html__( 'Up %s', 'omega-jewelry-store' ), '<span class="arrow" aria-hidden="true">&uarr;</span>' );
                                ?>
                            </span>
                        </a>
...
if( !function_exists( 'omega_jewelry_store_main_slider' ) ) :
    ...
    function omega_jewelry_store_main_slider(){
        ...
        if( $omega_jewelry_store_header_banner ){
            ...
            $banner_query = new WP_Query( array('post_type' => 'post', 'posts_per_page' => 4,'post__not_in' => get_option("sticky_posts"), 'category_name' => esc_html( $omega_jewelry_store_header_banner_cat ) ) );
...
if( !function_exists( 'omega_jewelry_store_product_section' ) ) :
    ...
    function omega_jewelry_store_product_section(){ 
        ...
        $product_section_title = esc_html( get_theme_mod( 'omega_jewelry_store_product_section_title',
        ...
        if ( class_exists( 'WooCommerce' ) ) {
            ...
            <div class="theme-product-block">
                <div class="wrapper">
                    <?php if( $product_section_title ){ ?>
                        <h3><?php echo esc_html( $product_section_title ); ?></h3>
                    <?php } ?>
...
if (!function_exists('omega_jewelry_store_post_format_icon')):
    ...
    function omega_jewelry_store_post_format_icon() {
        ...
        $title = '';
        if( $format == 'video' ){
            $icon = omega_jewelry_store_get_theme_svg( 'video' );
            $title = esc_html__('Video','omega-jewelry-store');
        }elseif( $format == 'audio' ){
            $icon = omega_jewelry_store_get_theme_svg( 'audio' );
            $title = esc_html__('Audio','omega-jewelry-store');
        }elseif( $format == 'gallery' ){
            $icon = omega_jewelry_store_get_theme_svg( 'gallery' );
            $title = esc_html__('Gallery','omega-jewelry-store');
        }elseif( $format == 'quote' ){
            $icon = omega_jewelry_store_get_theme_svg( 'quote' );
            $title = esc_html__('Quote','omega-jewelry-store');
        }elseif( $format == 'image' ){
            $icon = omega_jewelry_store_get_theme_svg( 'image' );
            $title = esc_html__('Image','omega-jewelry-store');
        }
        
        if (!empty($icon)) { ?>
            <div class="theme-post-format">
                <span class="post-format-icom"><?php echo omega_jewelry_store_svg_escape($icon); ?></span>
...
if ( ! function_exists( 'omega_jewelry_store_svg_escape' ) ):
     ...
     */
    function omega_jewelry_store_svg_escape( $input ) {
...
if( !function_exists( 'omega_jewelry_store_sanitize_sidebar_option_meta' ) ) :
    ...
    function omega_jewelry_store_sanitize_sidebar_option_meta( $input ){
